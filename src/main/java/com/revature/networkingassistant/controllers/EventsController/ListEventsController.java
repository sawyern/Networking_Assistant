...
public class ListEventsController {
    ...
    public ResponseEntity<Iterable<Event>> listEvents(@RequestBody JsonRequestBody requestBody, HttpServletResponse response) {
        SessionToken token = requestBody.getToken();
        if (tokenRepo.existsById(token.getId())) {
            return new ResponseEntity<Iterable<Event>>(eventRepo.findAll(), HttpStatus.OK);
        } else {
            return new ResponseEntity<Iterable<Event>>(eventRepo.findAll(), HttpStatus.UNAUTHORIZED);
        }
    }
}
